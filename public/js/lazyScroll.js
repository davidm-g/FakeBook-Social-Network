document.addEventListener('DOMContentLoaded', function() {;    
    if (window.location.pathname.includes("/search")) {
            let page = 1;
            let loading = false;
            window.noMoreResultsSearch = false;
            const searchResultsContainer = document.getElementById("search-results-container");

            function loadMoreResults() {
                if (window.noMoreResultsSearch || loading) return;

                if (
                    searchResultsContainer.scrollTop + searchResultsContainer.clientHeight >=
                        searchResultsContainer.scrollHeight - 100 && !loading
                ) {
                    loading = true;
                    page++;
                    ...
                    fetch(fetchUrl)
                        ...
                        .then((data) => {
                            ...
                            if (elements.length === 0) {
                                ...
                                window.noMoreResultsSearch = true;
                                loading = false;
                                document.getElementById("loading").style.display = "none";
                                return;
                            }
